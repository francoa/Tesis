...
\begin{frame}
    ...
    \begin{figure}
    \centering
    \includegraphics[width=11cm]{Presentacion/modifications_B.png}
    \end{figure}
  \end{textblock*}
    \begin{textblock*}{4cm}(0.5cm,6cm)
      \begin{figure}
      \centering
      \includegraphics[width=3.5cm]{Presentacion/porous.png}
      \end{figure}
    \end{textblock*}
...
\begin{frame}
    ...
    \vspace{0cm}
    Se busca imitar procedimientos experimentales (Zuo \textit{et al.}, 2003)
    \only<1>{
    \begin{enumerate}
        \item Ubicaci\'on aleatoria de part\'iculas de 2.5 nm de radio.\vspace{0.1cm}
        \item Estabilización a 650K (volumen constante, pocos ps).\vspace{0.1cm}
        \item Hasta 10 ps de compresi\'on (400 bar).\vspace{0.1cm}
        \item Repetición pasos 2 y 3 $\rightarrow$ \textbf{Extracción}\vspace{0.1cm}
    ...
    \only<2>{
	\begin{textblock*}{5cm}(1.5cm,3.5cm) % {block width} (coords)
            \includegraphics[width=5cm]{Cap_5/spheres2.png}
        \end{textblock*}
        \begin{textblock*}{5cm}(7cm,3.5cm) % {block width} (coords)
            \includegraphics[width=5cm]{Cap_5/spheres3.png}
        \end{textblock*}
        \begin{textblock*}{3cm}(3.8cm,8cm)
         \scriptsize{Previo al sinterizado}
        \end{textblock*}
        \begin{textblock*}{3cm}(8.1cm,8cm)
         \scriptsize{Posterior al sinterizado}         
        \end{textblock*}
        }
        
	\begin{textblock*}{12cm}(1.2cm,8.7cm)
	  \scriptsize{Zuo, R., Aulbach, E., and Rodel, J., \textit{Acta Mater.}, \textbf{51}, 4563-4574 (2003)}
	\end{textblock*}
\end{frame}
...
\begin{frame}
    ...
    \centering
        A mayor porosidad $\rightarrow$ menor esfuerzo para cerrar los poros (''radio efectivo'' mayor)
    \end{textblock*}
\end{frame}
...
\begin{frame}
    ...
    \begin{textblock*}{4.8cm}(0.5cm,6.5cm)
        El resultado es diferente al esperado.\\%Hay plasticidad pero los poliedros de Voronoi pr\'acticamente no cambian.\\
...
\begin{frame}
    ...
    \end{textblock*}
    \begin{textblock*}{11.5cm}(0.5cm,8.2cm) % {block width} (coords)
	\centering
        %Flujo pl\'astico.\\
...
\begin{frame}
 ...
 \begin{textblock*}{6cm}(5cm,7.1cm)
  ...
  \scriptsize{Tracción}
 \end{textblock*}
 \begin{textblock*}{12cm}(0.7cm,8.2cm)
 Aumento leve $\sigma$ máxima soportada y módulo de elasticidad.\\
